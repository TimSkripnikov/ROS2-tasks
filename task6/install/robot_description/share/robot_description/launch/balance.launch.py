...
def generate_launch_description():
    ...
    sdf_file = os.path.join(pkg_share, 'src', 'models', 'ballbot.sdf')
    ...
    gz_topic = '/model/ballbot'
    joint_state_gz_topic = '/world/ballbot' + gz_topic + '/joint_state'
    ...
    balance_pulisher = Node(
        package='robot_description',
        executable='balance.py',
        name='balance_pulisher',
        output='screen'
    )
    ...
    rviz = Node(
        package='rviz2',
        executable='rviz2',
        arguments=['-d', os.path.join(pkg_share, 'rviz', 'ballbot.rviz')],
        parameters=[
            {'use_sim_time': True},
        ]
    )
